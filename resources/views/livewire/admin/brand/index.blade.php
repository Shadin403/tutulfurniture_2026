                ...
                <div class="text-white shadow-lg card bg-gradient-dark">
                    <div class="p-4 card-body">
                        <div class="row align-items-center">
                            <div class="col">
                                <h2 class="mb-0 text-white fw-bold">ALL Brands List</h2>
                                <p class="mb-0 text-light">Manage your premium vehicle inventory</p>
                            </div>
                            <div class="col-auto">
                                <a href="{{ route('admin.brand.create') }}" class="px-4 py-2 shadow-sm btn btn-light"
                                    wire:navigate>
                        ...
                        <div class="row">
                            <div class="col-8">
                                <div class="numbers">
                                    ...
                                    <h5 class="mb-0 font-weight-bolder" id="total-cars">{{ $brands->count() }}</h5>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-8">
                                <div class="numbers">
                                    ...
                                    <h5 class="mb-0 font-weight-bolder" id="available-cars">5</h5>
                                </div>
                            </div>
                            <div class="col-4 text-end">
                                <div
                                ...
                                <tr wire:key="{{ $brand->id }}">
                                    ...
                                    <td>
                                        <img height="50px" width="50px"
                                            src="{{ asset('storage/uploads/brands/' . $brand->image) ?? asset('admin/img/no-image.png') }}"
                                            alt="{{ $brand->name }}">
                                    </td>
                                    <td>
                                        <p class="mb-0 text-sm">{{ $brand->slug }}</p>
                                    </td>
                                    <td>
                                        @php
                                            $words = explode(' ', $brand->description);
                                            if (count($words) > 50) {
                                                $brand->description = implode(' ', array_slice($words, 0, 50)) . '...';
                                            }
                                            if (!function_exists('html_trim_words')) {
                                                function html_trim_words($html, $limit = 50, $end = '...')
                                                {
                                                    $text = strip_tags($html); // HTML ট্যাগ বাদ
                                                    return Str::limit($text, $limit, $end); // Laravel এর string limiter
                                                }
                                            }
                                        @endphp
                                        <p class="mb-0 text-sm">{!! html_trim_words($brand->description, 50) !!}</p>
                                    </td>
                                    <td class="text-end pe-4">
                                        <a href="{{ route('admin.brand.edit', $brand->id) }}" wire:navigate.prefetch
                                        ...
                                        <a onclick="confirmDelete({{ $brand->id }})"
